...
const PaymentPage = () => {
    ...
    const handlePayment = () => {
        ...
        if (window.confirm(`Konfirmasi pembayaran ${service.service_name} senilai Rp ${service.service_tariff.toLocaleString('id-ID')}?`)) {
            dispatch(postTransaction(service.service_code)).then((result) => {
                if (result.meta.requestStatus === 'fulfilled') {
                    alert('Pembayaran Berhasil!');
                    dispatch(getBalance());
                    navigate('/transaction'); // Ke riwayat setelah bayar
                }
            });
        }
    };
